...
class MVS_API CameraIntern
{
  ...
  public:
    ...
    template <typename TYPE> static inline TMatrix<TYPE, 3, 3> ScaleK(const TMatrix<TYPE, 3, 3> &K, TYPE s)
    {
#if 0
		...
		return S*K;
#else
        return TMatrix<TYPE, 3, 3>(K(0, 0) * s, K(0, 1) * s, K(0, 2) * s, TYPE(0), K(1, 1) * s, K(1, 2) * s, TYPE(0),
                                   TYPE(0), TYPE(1));
#endif
    }
    ...
    template <typename TYPE, typename TYPER>
    static inline TMatrix<TYPE, 3, 3> ComposeInvK(const TYPE &fX, const TYPE &fY, TYPER w = TYPER(1),
                                                  TYPER h = TYPER(1))
    {
        ASSERT(w > 0 && h > 0);
    ...
    template <typename TYPE> inline TMatrix<TYPE, 3, 3> GetK(uint32_t width, uint32_t height) const
    {
        ...
        if (K(0, 2) == 0 && K(1, 2) == 0)
            return ComposeK(TYPE(K(0, 0) * fScale), TYPE(K(1, 1) * fScale), width, height);
    ...
    template <typename TYPE> inline TMatrix<TYPE, 3, 3> GetInvK(uint32_t width, uint32_t height) const
    {
        ASSERT(width > 0 && height > 0);
        const float fScale(GetNormalizationScale(width, height));
        if (K(0, 2) == 0 && K(1, 2) == 0)
            return ComposeInvK(TYPE(K(0, 0) * fScale), TYPE(K(1, 1) * fScale), width, height);
    ...
    template <typename TYPE> inline TPoint2<TYPE> NormalizeProjection(const TPoint2<TYPE> &proj) const
    {
        TPoint2<TYPE> pt;
...
class MVS_API Camera : public CameraIntern
{
  ...
  public:
    ...
    void Transform(const Matrix3x3 &R, const Point3 &t,
    ...
    static REAL StereoRectify(const cv::Size &size1, const Camera &camera1, const cv::Size &size2,
                              const Camera &camera2, Matrix3x3 &R1, Matrix3x3 &R2, Matrix3x3 &K1, Matrix3x3 &K2);
    static REAL StereoRectifyFusiello(const cv::Size &size1, const Camera &camera1, const cv::Size &size2,
                                      const Camera &camera2, Matrix3x3 &R1, Matrix3x3 &R2, Matrix3x3 &K1,
                                      Matrix3x3 &K2);
    static void SetStereoRectificationROI(const Point3fArr &points1, cv::Size &size1, const Camera &camera1,
                                          const Point3fArr &points2, cv::Size &size2, const Camera &camera2,
                                          const Matrix3x3 &R1, const Matrix3x3 &R2, Matrix3x3 &K1, Matrix3x3 &K2);
    ...
    template <typename TYPE> inline TPoint3<TYPE> ProjectPointP3(const TPoint3<TYPE> &X) const
    {
        ...
        return TPoint3<TYPE>((TYPE)(p[0 * 4 + 0] * X.x + p[0 * 4 + 1] * X.y + p[0 * 4 + 2] * X.z + p[0 * 4 + 3]),
    ...
    {
        return TPoint3<TYPE>(TYPE((x.x - K(0, 2)) / K(0, 0)), TYPE((x.y - K(1, 2)) / K(1, 1)), TYPE(1));
    }
    ...
    {
        return TPoint3<TYPE>(TYPE((X.x - K(0, 2)) * X.z / K(0, 0)), TYPE((X.y - K(1, 2)) * X.z / K(1, 1)), X.z);
    }
    ...
    template <typename TYPE> inline bool IsInside(const TPoint2<TYPE> &pt, const TPoint2<TYPE> &size) const
    {
        return pt.x >= 0 && pt.y >= 0 && pt.x < size.x && pt.y < size.y;
    }
    template <typename TYPE> inline bool IsInsideProjection(const TPoint3<TYPE> &X, const TPoint2<TYPE> &size) const
    {
        return IsInside(ProjectPoint(X), size);
    }
    template <typename TYPE> inline bool IsInsideProjectionP(const TPoint3<TYPE> &X, const TPoint2<TYPE> &size) const
    {
        return IsInside(ProjectPointP(X), size);
    }

    // same as above, but for ortho-projection
    template <typename TYPE> inline TPoint3<TYPE> TransformPointOrthoI2C(const TPoint3<TYPE> &x) const
    {
        return TPoint3<TYPE>(TYPE((x.x - K(0, 2)) / K(0, 0)), TYPE((x.y - K(1, 2)) / K(1, 1)), x.z);
    }
    template <typename TYPE> inline TPoint3<TYPE> TransformPointOrthoI2W(const TPoint3<TYPE> &x) const
    {
        return TransformPointC2W(TransformPointOrthoI2C(x));
    }
    template <typename TYPE> inline TPoint2<TYPE> TransformPointOrthoC2I(const TPoint3<TYPE> &X) const
    {
        return TransformPointC2I(TPoint2<TYPE>(X.x, X.y));
    }
    template <typename TYPE> inline TPoint2<TYPE> TransformPointOrthoW2I(const TPoint3<TYPE> &X) const
    {
