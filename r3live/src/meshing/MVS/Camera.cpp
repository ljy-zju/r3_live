...
// decomposition of projection matrix into KR[I|-C]: internal calibration ([3,3]), rotation ([3,3]) and translation
// ([3,1]) (comparable with OpenCV: normalized cv::decomposeProjectionMatrix)
...
void GetImagePairROI(const Point3fArr &points1, const Point3fArr &points2, const Matrix3x3 &K1, const Matrix3x3 &K2,
                     const Matrix3x3 &R1, const Matrix3x3 &R2, const Matrix3x3 &invK1, const Matrix3x3 &invK2,
                     AABB2f &roi1h, AABB2f &roi2h)
{
...
void SetCameraMatricesROI(const AABB2f &roi1h, const AABB2f &roi2h, cv::Size &size1, cv::Size &size2, Matrix3x3 &K1,
                          Matrix3x3 &K2)
{
...
REAL Camera::StereoRectify(const cv::Size &size1, const Camera &camera1, const cv::Size &size2, const Camera &camera2,
                           Matrix3x3 &R1, Matrix3x3 &R2, Matrix3x3 &K1, Matrix3x3 &K2)
{
...
REAL Camera::StereoRectifyFusiello(const cv::Size &size1, const Camera &camera1, const cv::Size &size2,
...
// adjust rectified camera matrices such that the entire area common to both source images is contained in the rectified
// images; as long as the focal-length on x and the skewness are equal in both cameras the point-scale is also equal in
// both images; note however that the surface-scale can still be different (most likely is) as it depends on its
// (unknown) normal too;
//  - points1 and points2: contain the pairs of corresponding pairs of image projections and their depth
//  - size1 and size2: input the size of the source images, output the size of the rectified images (rectified image
//  sizes are equal)
void Camera::SetStereoRectificationROI(const Point3fArr &points1, cv::Size &size1, const Camera &camera1,
