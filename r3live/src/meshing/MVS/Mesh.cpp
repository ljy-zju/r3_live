...
#else
    reportCudaError(kernelComputeFaceNormal((int)faces.GetSize(), vertices, faces,
                                            CUDA::KernelRT::OutputParam(faceNormals.GetDataSize()), faces.GetSize()));
...
#define IS_LOOP_FACE3(a, b, c) (v##a##2 == v##b##1 && v##b##2 == v##c##1 && v##c##2 == v##a##1)
#define IS_LINK_FACE3(a, b, c) (v##a##2 == v##b##1 && v##b##2 == v##c##1)
...
#define DEFINE_REMOVE3(go2)                                                                                            \
    ...
    faces.AddConstruct(v01, v02, (v02 == v11 ? v12 : v22));                                                            \
...
#define DEFINE_REMOVE4(go2)                                                                                            \
    ...
    else DEFINE_FACES4(0, 1, 3, 2, go2) else DEFINE_FACES4(0, 2, 1, 3, go2) else DEFINE_FACES4(                        \
        0, 2, 3, 1, go2) else DEFINE_FACES4(0, 3, 1, 2, go2) else DEFINE_FACES4(0, 3, 2, 1, go2) else goto go2;        \
...
#define DEFINE_REMOVE_FACES                                                                                            \
    ...
    {                                                                                                                  \
        /* remove old faces; */                                                                                        \
        /* delete them in reverse order since the remove operation is simply replacing the removed item with the last  \
         * item */                                                                                                     \
...
bool Mesh::FixNonManifold()
{
                ...
                }
                {
                    ...
                    FOREACH(s, sizes)
                    {
                        ...
                        for (const auto &comp : vertexInfo.components)
                        {
                            if (comp.second == s)
                            {
                                for (boost::tie(ei, eie) = boost::out_edges(comp.first, vertexInfo.graph); ei != eie;
                                     ++ei)
                        ...
                        {
                            // this is the case of a legitimate vertex on the surface and a pyramid rising from the
                            // neighboring surface having the apex the current vertex and the base formed by 3 vertices
                            // - one being this vertex;
                        ...
                        {
                            // this is the case of a legitimate vertex on the surface and a pyramid rising from the
                            // neighboring surface having the apex the current vertex and the base formed by 4 vertices
                            // - one being this vertex;
    ...
    FOREACH(v, vertices)
    {
        ...
        }
        {
            ...
            FOREACH(s, sizes)
            {
                ...
                {
                    // this is the case of a legitimate vertex on the surface and a pyramid rising from the neighboring
                    // surface having the apex this vertex and the base formed by 3 vertices;
                ...
                {
                    // this is the case of a legitimate vertex on the surface and a pyramid rising from the neighboring
                    // surface having the apex this vertex and the base formed by 4 vertices;
    ...
    if (nNonManifoldVertices)
        vertexFaces.Empty();
    DEFINE_REMOVE_FACES;
    // DEBUG_ULTIMATE( "Fixed %u/%u non-manifold edges/vertices and %u faces removed: %u pyramid3 and %u pyramid4 (%s)",
...
class Vertex : public vcg::Vertex<UsedTypes, vcg::vertex::Coord3f, vcg::vertex::Normal3f, vcg::vertex::VFAdj,
                                  vcg::vertex::Mark, vcg::vertex::BitFlags>
{
};
class Face : public vcg::Face<UsedTypes, vcg::face::VertexRef, vcg::face::Normal3f, vcg::face::FFAdj, vcg::face::VFAdj,
                 ...
                 bool bLastClean)
{
    ...
    if (fDecimate < 1)
    {
        ...
        pp.QualityThr =
            0.3; // Quality Threshold for penalizing bad shaped faces: the value is in the range [0..1], 0 accept any
                 // kind of face (no penalties), 0.5 penalize faces with quality < 0.5, proportionally to their shape
        pp.PreserveBoundary =
            false; // the simplification process tries to not affect mesh boundaries during simplification
        pp.BoundaryWeight =
            1; // the importance of the boundary during simplification: the value is in the range (0..+inf), default
               // (1.0) means that the boundary has the same importance as the rest; values greater than 1.0 raise
               // boundary importance and has the effect of removing less vertices on the border
        pp.PreserveTopology =
            false; // avoid all collapses that cause a topology change in the mesh (like closing holes, squeezing
                   // handles, etc); if checked the genus of the mesh should stay unchanged
        pp.QualityWeight =
            false; // use the Per-Vertex quality as a weighting factor for the simplification: the weight is used as an
                   // error amplification value, so a vertex with a high quality value will not be simplified and a
                   // portion of the mesh with low quality values will be aggressively simplified
        pp.NormalCheck =
            false; // try to avoid face flipping effects and try to preserve the original orientation of the surface
        pp.OptimalPlacement =
            true; // each collapsed vertex is placed in the position minimizing the quadric error; it can fail (creating
                  // bad spikes) in case of very flat areas; if disabled edges are collapsed onto one of the two
                  // original vertices and the final mesh is composed by a subset of the original vertices
        pp.QualityQuadric = false; // add additional simplification constraints that improves the quality of the
    ...
    if (fSpurious > 0)
    {
        ...
        vcg::tri::UpdateTopology<CLEAN::Mesh>::FaceFace(mesh);
        const std::pair<int, int> delInfo(
            vcg::tri::Clean<CLEAN::Mesh>::RemoveSmallConnectedComponentsDiameter(mesh, thLongEdge));
    ...
    if (nCloseHoles > 0)
    {
        ...
        const int holeCnt(
            vcg::tri::Hole<CLEAN::Mesh>::EarCuttingIntersectionFill<vcg::tri::SelfIntersectionEar<CLEAN::Mesh>>(
                mesh, (int)nCloseHoles, false));
#else
        const int holeCnt = vcg::tri::Hole<CLEAN::Mesh>::EarCuttingFill<vcg::tri::MinimumWeightEar<CLEAN::Mesh>>(
            mesh, (int)nCloseHoles, false);
#endif
    ...
    // DEBUG( "Cleaned mesh: %u vertices, %u faces (%s)", vertices.GetSize(), faces.GetSize(),
    // TD_TIMER_GET_FMT().c_str() );
    printf("Cleaned mesh: %u vertices, %u faces (%s)\r\n", vertices.GetSize(), faces.GetSize(),
           TD_TIMER_GET_FMT().c_str());
} // Clean
/*----------------------------------------------------------------*/
...
static const PLY::PlyProperty vert_props[] = {{"x", PLY::Float32, PLY::Float32, offsetof(Mesh::Vertex, x), 0, 0, 0, 0},
...
static const PLY::PlyProperty face_props[] = {{"vertex_indices", PLY::Uint32, PLY::Uint32, offsetof(Face, pFace), 1,
                                               PLY::Uint8, PLY::Uint8, offsetof(Face, num)}};
struct TexCoord
{
    uint8_t num;
    Mesh::TexCoord *pTex;
...
static const PLY::PlyProperty face_tex_props[] = {{"vertex_indices", PLY::Uint32, PLY::Uint32,
                                                   offsetof(FaceTex, face.pFace), 1, PLY::Uint8, PLY::Uint8,
                                                   offsetof(FaceTex, face.num)},
                                                  {"texcoord", PLY::Float32, PLY::Float32, offsetof(FaceTex, tex.pTex),
                                                   1, PLY::Uint8, PLY::Uint8, offsetof(FaceTex, tex.num)}};
...
bool Mesh::Load(const String &fileName)
{
    ...
    if (!ret)
        return false;
    DEBUG_EXTRA("Mesh loaded: %u vertices, %u faces (%s)", vertices.GetSize(), faces.GetSize(),
                TD_TIMER_GET_FMT().c_str());
    return true;
}
...
bool Mesh::Save(const String &fileName, const cList<String> &comments, bool bBinary) const
{
    ...
    if (!ret)
        return false;
    // DEBUG_EXTRA( "Mesh saved: %u vertices, %u faces (%s)", vertices.GetSize(), faces.GetSize(),
    // TD_TIMER_GET_FMT().c_str() );
...
bool Mesh::SavePLY(const String &fileName, const cList<String> &comments, bool bBinary) const
{
    ...
    // create PLY object
    const size_t bufferSize(vertices.GetSize() * (4 * 3 /*pos*/ + 2 /*eol*/) +
                            faces.GetSize() * (1 * 1 /*len*/ + 4 * 3 /*idx*/ + 2 /*eol*/) + 2048 /*extra size*/);
    PLY ply;
...
template <class Refs, class T, class P, class Normal> class MeshVertex : public CGAL::HalfedgeDS_vertex_base<Refs, T, P>
{
  public:
...
template <class Refs, class T, class Normal> class MeshFacet : public CGAL::HalfedgeDS_face_base<Refs, T>
{
  public:
...
static bool CanFlipEdge(Vertex::Halfedge_handle h)
{
    ...
    Vector v_perp_2 = CGAL::cross_product(h->opposite()->next()->vertex()->point() - h->opposite()->vertex()->point(),
                                          h->vertex()->point() - h->prev()->vertex()->point());
    ...
    case 3: {
        ...
        do
        {
            ...
            for (Edge_iterator ei = p.edges_begin(); ei != p.edges_end(); ++ei)
            {
                ...
                float cost1(
                    (float)MINF(MINF(MINF(MINF(MINF(p_angle(p3, p1, p2), p_angle(p1, p3, p2)), p_angle(p4, p1, p3)),
                                          p_angle(p4, p3, p1)),
                                     p_angle(p1, p4, p2)),
                                p_angle(p1, p2, p3)));
                float cost2(
                    (float)MINF(MINF(MINF(MINF(MINF(p_angle(p1, p2, p4), p_angle(p1, p4, p2)), p_angle(p3, p4, p2)),
                                          p_angle(p4, p2, p3)),
                                     p_angle(p4, p1, p2)),
                                p_angle(p4, p3, p2)));
...
//  - collapseRatio
//     - for small angles: given the corresponding edges (a,b,c) in all permutations, if (a/b < collapseRatio) & (a/c <
//     collapseRatio)
...
static int FixDegeneracy(Polyhedron &p, double collapseRatio, double degenerateAngleDeg)
{
    ...
    for (Facet_iterator fi = p.facets_begin(); fi != p.facets_end();)
    {
        ...
        if (fi->triangle().is_degenerate())
        {
            ...
            DEBUG_LEVEL(3, "Degenerate angle: %g (avg edge %g)",
                        v_angle(fi->get_point(0) - fi->get_point(1), fi->get_point(0) - fi->get_point(2)), avg_edge);
            const double delta(avg_edge * 0.01);
                ...
                fi->halfedge()->next()->next()->vertex()->point() + Vector(delta, 0, 0);
        }
...
// - The method also attempts to fix degeneracies by invoking FixDegeneracy(collapseRatio,degenerate_angle_deg) and
// performs some local smoothing, based on the operating mode. Parameters:
// - [epsilonMin, epsilonMax] - the desired edge interval (negative if to be used as multiplier of the initial mean edge
// length)
...
// - max_iter (default=30) - maximum number of iterations to be performed; since there is no guarantee that one
// operations (such as a collapse, for example)
//   will not in turn generate new degeneracies, operations are being performed on the mesh in an iterative fashion.
static void EnsureEdgeSize(Polyhedron &p, double epsilonMin, double epsilonMax, double collapseRatio,
                           double degenerate_angle_deg, int mode, int max_iters, int comp_size_threshold)
{
    if (mode > 0)
    ...
    DEBUG_LEVEL(3, "Ensuring edge size in [%g, %g] with edges currently in [%g, %g]", epsilonMin, epsilonMax, edge.min,
                edge.max);
    ...
    {
        ComputeStatsEdge(p, edge);
        VERBOSE("Edge size in [%g, %g] (requested in [%g, %g]): %d ops, %d iters", edge.min, edge.max, epsilonMin,
                epsilonMax, total_no_ops, iters);
    }
#endif
}
...
#if TD_VERBOSE != TD_VERBOSE_OFF
        if (bProblems)
            DEBUG_EXTRA("warning: ignoring the following facet(s) violating the manifold constraint:%s",
                        msgFaces.c_str());
#endif
...
static bool ImportMesh(Polyhedron &p, const Mesh::VertexArr &vertices, const Mesh::FaceArr &faces)
{
    ...
    p.delegate(builder);
    UpdateMeshData(p);
    DEBUG_ULTIMATE("Mesh imported: %u vertices, %u facets (%u border edges)", p.size_of_vertices(), p.size_of_facets(),
...
static bool ExportMesh(const Polyhedron &p, Mesh::VertexArr &vertices, Mesh::FaceArr &faces)
{
    ...
    DEBUG_ULTIMATE("Mesh exported: %u vertices, %u facets (%u border edges)", p.size_of_vertices(), p.size_of_facets(),
                   p.size_of_border_edges());
    return true;
}
} // namespace CLN

void Mesh::EnsureEdgeSize(float epsilonMin, float epsilonMax, float collapseRatio, float degenerate_angle_deg, int mode,
                          int max_iters)
{
    CLN::Polyhedron p;
...
void Mesh::CloseHoleQuality(VertexIdxArr &verts)
{
    struct CandidateFace
    {
        ...
        CandidateFace(VIndex v0, VIndex v1, VIndex v2, const Mesh &mesh) : face(v0, v1, v2)
        {
            ...
            angle = ACOS(ComputeAngle<float, float>(mesh.vertices[face[1]].ptr(), mesh.vertices[face[0]].ptr(),
            ...
            else
            {
                ...
                else
                {
                    ...
                    dihedral = MAXF(ACOS(ComputeAngle<float, float>(n.ptr(), n0.ptr())),
                                    ACOS(ComputeAngle<float, float>(n.ptr(), n1.ptr())));
                }
            }
            aspectRatio =
                ComputeTriangleQuality(mesh.vertices[face[0]], mesh.vertices[face[1]], mesh.vertices[face[2]]);
        }
    ...
    {
        if (candidateFaces
                .AddConstruct(verts[v], verts[(v + 1) % verts.GetSize()], verts[(v + 2) % verts.GetSize()], *this)
                .aspectRatio < 0)
