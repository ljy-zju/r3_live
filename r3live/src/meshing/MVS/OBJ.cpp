...
ObjModel::MaterialLib::Material::Material(const Image8U3 &_diffuse_map, const Color &_Kd)
    : diffuse_map(_diffuse_map), Kd(_Kd)
{
}
...
bool ObjModel::Save(const String &fileName, unsigned precision, bool texLossless) const
{
    ...
    {
        out << "vt " << texcoords[i][0] << " " << texcoords[i][1] << "\n";
    }
...
{
    MaterialLib::Materials::iterator it(
        std::find_if(material_lib.materials.begin(), material_lib.materials.end(),
                     [&name](const MaterialLib::Material &mat) { return mat.name == name; }));
