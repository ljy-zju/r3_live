...
template <bool sTree> void IBFSGraph::augmentTree(Node *x, EdgeCap bottleneck)
{
    Node *y;
    Arc *a;
    ...
    for (int level = 2; level <= orphanBuckets.maxBucket; level++)
    {
        while ((x = orphanBuckets.popFront(level)) != NULL)
        {
            ...
            if (x->parent == NULL)
            {
                ...
                {
                    y = a->head;
                    if ((sTree ? a->isRevResidual : a->rCap) && (y->excess || y->parent != NULL) &&
                        //! y->isOrphan() &&
                        (sTree ? (y->label > 0) : (y->label < 0)) &&
                        (sTree ? (y->label < minLabel) : (y->label > minLabel)))
                    {
...
template <bool dirS> void IBFSGraph::growth()
{
    ...
    for (Node **active = active0.list; active != (active0.list + active0.len); active++)
    {
        ...
        {
            stats.incGrowthArcs();
            if ((dirS ? a->rCap : a->isRevResidual) == 0)
                continue;
            y = a->head;
            if (y->label == numNodes)
            {
...
void IBFSGraph::testTree()
{
    ...
    for (x = nodes; x != nodeEnd; x++)
    {
        ...
        for (a = x->firstArc; a != (x + 1)->firstArc; a++)
        {
            if (x->isParentCurr && (sTree ? a->isRevResidual : a->rCap) &&
                (sTree ? (a->head->label > 0) : (a->head->label < 0)) &&
                a->head->label == (sTree ? (x->label - 1) : (x->label + 1)) && a < x->parent)
            {
...
EdgeCap IBFSGraph::computeMaxFlow()
{
    ...
    while (true)
    {
        ...
        if ((!IB_ALTERNATE_SMART && dirS) || (IB_ALTERNATE_SMART && uniqOrphansT == uniqOrphansS && dirS) ||
...
bool IBFSGraph::readFromFile(char *filename, bool checkCompile)
{
    ...
    while (fgets(line, MAX_LINE_LEN, pFile) != NULL)
    {
        ...
        case 'a':
            sscanf(line, "%c %d %d %d %d", &c, &nodeId1, &nodeId2, &capacity, &capacity2);
            if (nodeId1 < 0 || nodeId1 >= declaredNumOfNodes || nodeId2 < 0 || nodeId2 >= declaredNumOfNodes)
            {
