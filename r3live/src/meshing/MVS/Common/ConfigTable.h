...
class GENERAL_API CConfigTable
{
  public:
    ...
    inline SMLVALUE &operator[](const String &name)
    {
        return m_oSML[name];
    }
    inline IDX InsertChild(CConfigTable &oCfg)
    {
        oCfg.SetParent(this);
        return m_oSML.InsertChild((const LPSML)&oCfg.m_oSML);
    }
    inline void RemoveChild(CConfigTable &oCfg)
    {
        oCfg.SetParent(NULL);
        m_oSML.RemoveChild(oCfg.m_oSML.GetName());
    }
    ...
    {
        return m_oSML.GetName();
    }
    inline CConfigTable *GetParent() const
    {
        return m_pParent;
    }
    inline void SetParent(CConfigTable *pParent)
    {
        m_pParent = pParent;
    }
