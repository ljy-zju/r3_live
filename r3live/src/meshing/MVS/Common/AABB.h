...
template <typename TYPE, int DIMS> class TAABB
{
  ...
  public:
    ...
    enum
    {
        numChildren = (2 << (DIMS - 1))
    };
    enum
    {
        numCorners = (DIMS == 1 ? 2 : (DIMS == 2 ? 4 : 8))
    }; // 2^DIMS
    enum
    {
        numScalar = (2 * DIMS)
    ...
    template <typename TPoint> inline TAABB(const TPoint *pts, size_t n);
    template <typename CTYPE> inline TAABB(const TAABB<CTYPE, DIMS> &);
    ...
    template <typename TPoint> inline void Set(const TPoint *pts, size_t n);
    ...
    inline TYPE &operator[](BYTE i)
    {
        ASSERT(i < numScalar);
        return ptMin.data()[i];
    }
    inline TYPE operator[](BYTE i) const
    {
        ASSERT(i < numScalar);
        return ptMin.data()[i];
    }
