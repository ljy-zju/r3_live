...
#endif
#define INIT_WORKING_FOLDER                                                                                            \
    {                                                                                                                  \
        SEACAVE::Util::ensureValidFolderPath(WORKING_FOLDER);                                                          \
        WORKING_FOLDER_FULL = SEACAVE::Util::getFullPath(WORKING_FOLDER);                                              \
    } // initialize working folders
#define MAKE_PATH(str)                                                                                                 \
    SEACAVE::Util::getSimplifiedPath(WORKING_FOLDER + (str)) // add working directory to the given file name
#define MAKE_PATH_SAFE(str)                                                                                            \
    (SEACAVE::Util::isFullPath(str)                                                                                    \
         ? SEACAVE::String(str)                                                                                        \
         : MAKE_PATH(str)) // add working directory to the given file name only if not full path already
#define MAKE_PATH_FULL(p, s)                                                                                           \
    (SEACAVE::Util::isFullPath((s).c_str())                                                                            \
         ? SEACAVE::String(s)                                                                                          \
         : SEACAVE::Util::getSimplifiedPath((p) + (s))) // add the given path to the given file name
#define MAKE_PATH_REL(p, s)                                                                                            \
    ((s).compare(0, (p).size(), p)                                                                                     \
         ? SEACAVE::String(s)                                                                                          \
         : SEACAVE::String(SEACAVE::String((s).substr((p).size())))) // remove the given path from the given file name
#define GET_PATH_FULL(str)                                                                                             \
    (SEACAVE::Util::isFullPath((str).c_str())                                                                          \
         ? SEACAVE::Util::getFilePath(str)                                                                             \
         : SEACAVE::Util::getSimplifiedPath(                                                                           \
               WORKING_FOLDER_FULL + SEACAVE::Util::getFilePath(str))) // retrieve the full path to the given file

// macros simplifying the task of managing options
#define DECOPT_SPACE(SPACE)                                                                                            \
    namespace SPACE                                                                                                    \
    {                                                                                                                  \
    void init();                                                                                                       \
    void update();                                                                                                     \
...
#define DEFOPT_SPACE(SPACE, name)                                                                                      \
    namespace SPACE                                                                                                    \
    {                                                                                                                  \
...
#define DEFVAR_OPTION(SPACE, flags, type, name, title, desc, ...)                                                      \
    namespace SPACE                                                                                                    \
    {                                                                                                                  \
    type name;                                                                                                         \
...
#define FDEFVAR_string(SPACE, flags, name, title, desc, ...)                                                           \
    DEFVAR_OPTION(SPACE, flags, String, name, title, desc, __VA_ARGS__)
#define FDEFVAR_bool(SPACE, flags, name, title, desc, ...)                                                             \
    DEFVAR_OPTION(SPACE, flags, bool, name, title, desc, __VA_ARGS__)
#define FDEFVAR_int32(SPACE, flags, name, title, desc, ...)                                                            \
    DEFVAR_OPTION(SPACE, flags, int32_t, name, title, desc, __VA_ARGS__)
#define FDEFVAR_uint32(SPACE, flags, name, title, desc, ...)                                                           \
    DEFVAR_OPTION(SPACE, flags, uint32_t, name, title, desc, __VA_ARGS__)
#define FDEFVAR_flags(SPACE, flags, name, title, desc, ...)                                                            \
    DEFVAR_OPTION(SPACE, flags, Flags, name, title, desc, __VA_ARGS__)
#define FDEFVAR_float(SPACE, flags, name, title, desc, ...)                                                            \
    DEFVAR_OPTION(SPACE, flags, float, name, title, desc, __VA_ARGS__)
#define FDEFVAR_double(SPACE, flags, name, title, desc, ...)                                                           \
    DEFVAR_OPTION(SPACE, flags, double, name, title, desc, __VA_ARGS__)

#define DEFVAR_string(SPACE, name, title, desc, ...)                                                                   \
    DEFVAR_OPTION(SPACE, CFGITEM::NA, String, name, title, desc, __VA_ARGS__)
#define DEFVAR_bool(SPACE, name, title, desc, ...)                                                                     \
    DEFVAR_OPTION(SPACE, CFGITEM::NA, bool, name, title, desc, __VA_ARGS__)
#define DEFVAR_int32(SPACE, name, title, desc, ...)                                                                    \
    DEFVAR_OPTION(SPACE, CFGITEM::NA, int32_t, name, title, desc, __VA_ARGS__)
#define DEFVAR_uint32(SPACE, name, title, desc, ...)                                                                   \
    DEFVAR_OPTION(SPACE, CFGITEM::NA, uint32_t, name, title, desc, __VA_ARGS__)
#define DEFVAR_flags(SPACE, name, title, desc, ...)                                                                    \
    DEFVAR_OPTION(SPACE, CFGITEM::NA, Flags, name, title, desc, __VA_ARGS__)
#define DEFVAR_float(SPACE, name, title, desc, ...)                                                                    \
    DEFVAR_OPTION(SPACE, CFGITEM::NA, float, name, title, desc, __VA_ARGS__)
#define DEFVAR_double(SPACE, name, title, desc, ...)                                                                   \
    DEFVAR_OPTION(SPACE, CFGITEM::NA, double, name, title, desc, __VA_ARGS__)

#define TDEFVAR_string(SPACE, name, title, desc, ...)                                                                  \
    DEFVAR_OPTION(SPACE, CFGITEM::TEMP, String, name, title, desc, __VA_ARGS__)
#define TDEFVAR_bool(SPACE, name, title, desc, ...)                                                                    \
    DEFVAR_OPTION(SPACE, CFGITEM::TEMP, bool, name, title, desc, __VA_ARGS__)
#define TDEFVAR_int32(SPACE, name, title, desc, ...)                                                                   \
    DEFVAR_OPTION(SPACE, CFGITEM::TEMP, int32_t, name, title, desc, __VA_ARGS__)
#define TDEFVAR_uint32(SPACE, name, title, desc, ...)                                                                  \
    DEFVAR_OPTION(SPACE, CFGITEM::TEMP, uint32_t, name, title, desc, __VA_ARGS__)
#define TDEFVAR_flags(SPACE, name, title, desc, ...)                                                                   \
    DEFVAR_OPTION(SPACE, CFGITEM::TEMP, Flags, name, title, desc, __VA_ARGS__)
#define TDEFVAR_float(SPACE, name, title, desc, ...)                                                                   \
    DEFVAR_OPTION(SPACE, CFGITEM::TEMP, float, name, title, desc, __VA_ARGS__)
#define TDEFVAR_double(SPACE, name, title, desc, ...)                                                                  \
    DEFVAR_OPTION(SPACE, CFGITEM::TEMP, double, name, title, desc, __VA_ARGS__)
