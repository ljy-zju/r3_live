...
class Event
{
  public:
    ...
    virtual bool Run(void * /*pArgs*/ = NULL)
    {
        return true;
    }
...
class GENERAL_API EventQueue
{
  public:
    ...
    Event *GetEvent(
        uint32_t millis);  // block until an event arrives or time expires and get the first event pending in the queue
    Event *GetEventLast(); // block until an event arrives and get the last event pending in the queue
    Event *GetEventLast(
        uint32_t millis); // block until an event arrives or time expires and get the last event pending in the queue
...
class GENERAL_API EventThreadPool : public ThreadPool, public EventQueue
{
  public:
    ...
    inline EventThreadPool(size_type nThreads, Thread::FncStart pfnStarter, void *pData = NULL)
        : ThreadPool(nThreads, pfnStarter, pData)
    {
    }
    inline ~EventThreadPool()
    {
