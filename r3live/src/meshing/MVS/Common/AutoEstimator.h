...
/// tabulate logcombi(.,n)
template <typename Type> static inline void makelogcombi_n(size_t n, std::vector<Type> &l)
{
    l.resize(n + 1);
...
/// tabulate logcombi(k,.)
template <typename Type, int k> static inline void makelogcombi_k(size_t nmax, std::vector<Type> &l)
{
    l.resize(nmax + 1);
...
struct ErrorIndex
{
    ...
    inline bool operator==(const ErrorIndex &r) const
    {
...
static ErrorIndex bestNFA(double logalpha0, const ErrorIndexArr &e, double loge0, double maxThresholdSq,
                          const std::vector<float> &logc_n, const std::vector<float> &logc_k, double multError = 1.0)
{
    ...
    for (size_t k = NSAMPLES + 1; k <= n && e[k - 1].first <= maxThresholdSq; ++k)
    {
        ...
        const double NFA = loge0 + logalpha * (double)(k - NSAMPLES) + logc_n[k] + logc_k[k];
...
{
    template <size_t NSAMPLES> inline void Sample(size_t n, size_t *samples)
    {
        ASSERT(NSAMPLES > 0);
...
template <typename TMat, typename TRows> TMat ExtractRows(const TMat &A, const TRows &rows)
{
...
std::pair<double, double> ACRANSAC(Kernel &kernel, Sampler &sampler, std::vector<size_t> &vec_inliers,
                                   typename Kernel::Model &model, double maxThreshold = DBL_MAX,
...
#endif
        for (typename Kernel::Models::const_iterator itModel = vec_models.cbegin(); itModel != vec_models.cend();
             ++itModel)
        {
            ...
            ErrorIndex best = bestNFA<Kernel::MINIMUM_SAMPLES>(kernel.logalpha0(), vec_residuals, loge0, maxThresholdSq,
            ...
            if (best.first < minNFA /*&& vec_residuals[best.second-1].first < errorSqMax*/)
            {
                ...
                model = *itModel;
                DEBUG_LEVEL(4, "\titer=% 3u inliers=%u threshold=%g nfa=%g", iter, best.second, SQRT(errorSqMax),
                            minNFA);
#ifdef ACRANSAC_SAMPLE_INLIERS
                better = true;
#else
                nIter = cvRANSACUpdateNumIters(confidence, (double)(nData - best.second) / nData,
                                               unsigned(Kernel::MINIMUM_SAMPLES), (unsigned)nTrialsRound);
#endif
            }
        }
        vec_models.clear();
...
template <typename Kernel, typename Sampler>
void RANSAC(Kernel &kernel, Sampler &sampler, std::vector<size_t> &vec_inliers, typename Kernel::Model &model,
            double threshold,
#ifndef ACRANSAC_SAMPLE_INLIERS
            double confidence = 0.9999,
#endif
...
// Main estimation loop
    ...
    vec_inliers_tmp.reserve(nData);
    vec_inliers.reserve(nData);
    for (size_t iter = 0; iter < nIter || iter < nMinIter; ++iter)
    {
        // Get random sample
        sampler.template Sample<Kernel::MINIMUM_SAMPLES>(
...
#else
#ifdef _USE_MSAC
                nIter = cvRANSACUpdateNumIters(confidence, (double)score / ((double)nData * thresholdSq),
                                               unsigned(Kernel::MINIMUM_SAMPLES), (unsigned)nTrialsRound);
#else
                nIter = cvRANSACUpdateNumIters(confidence, (double)(nData - vec_inliers.size()) / nData,
                                               unsigned(Kernel::MINIMUM_SAMPLES), (unsigned)nTrialsRound);
#endif
#endif
            }
...
///  estimation.
template <typename SOLVER> class ACKernelAdaptor
{
  public:
    ...
    ACKernelAdaptor(const DMatrix32F &x1, float w1, float h1, const DMatrix32F &x2, float w2, float h2,
                    bool bPointToLine = true)
        : x1_(x1), x2_(x2), bPointToLine_(bPointToLine)
    {
        ASSERT(2 == x1_.cols);
    ...
    {
        return Solver::Error(model2evaluate, *x1_.ptr<const Point2f>((int)sample),
                             *x2_.ptr<const Point2f>((int)sample));
    }

    inline size_t NumSamples() const
    {
        return static_cast<size_t>(x1_.rows);
    }
    inline double logalpha0() const
    {
        return logalpha0_;
    }
    inline double multError() const
    {
        return (bPointToLine_ ? 0.5 : 1.0);
    }
...
template <typename SOLVER> class ACKernelAdaptorResection
{
  public:
    typedef SOLVER Solver;
    typedef typename SOLVER::Model Model;
    ...
    ACKernelAdaptorResection(const DMatrix32F &x2d, const DMatrix &x3D) : x2d_(x2d), x3D_(x3D), logalpha0_(log10(M_PI))
    {
        ASSERT(2 == x2d_.cols);
        ASSERT(3 == x3D_.cols);
    ...
    inline double Error(size_t sample) const
    {
        return Solver::Error(model2evaluate, *x2d_.ptr<const Point2f>((int)sample),
                             *x3D_.ptr<const Point3>((int)sample));
    }
...
template <typename SOLVER> class ACKernelAdaptorEssential
{
  public:
    typedef SOLVER Solver;
    typedef typename SOLVER::Model Model;
    ...
    ACKernelAdaptorEssential(const DMatrix32F &x1, float w1, float h1, const Matrix3x3f &invK1, const DMatrix32F &x2,
                             float w2, float h2, const Matrix3x3f &invK2)
        : solver(invK1, invK2), x1_(x1), x2_(x2)
    {
        ASSERT(2 == x1_.cols);
        ASSERT(x1_.rows == x2_.rows);
        ASSERT(x1_.cols == x2_.cols);
