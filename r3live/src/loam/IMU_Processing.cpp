                                           ...
                                           double end_pose_dt)
{
    ...
    for (std::deque<sensor_msgs::Imu::ConstPtr>::iterator it_imu = v_imu.begin(); it_imu != (v_imu.end() - 1); it_imu++)
    {
        ...
        // cov_w.block<3, 3>(18, 18).diagonal() = Eigen::Vector3d(COV_NOISE_EXT_I2C_R, COV_NOISE_EXT_I2C_R,
        // COV_NOISE_EXT_I2C_R) * dt * dt; // bias gyro covariance cov_w.block<3, 3>(21, 21).diagonal() =
        // Eigen::Vector3d(COV_NOISE_EXT_I2C_T, COV_NOISE_EXT_I2C_T, COV_NOISE_EXT_I2C_T) * dt
    ...
    // cout <<__FILE__ << ", " << __LINE__ <<" ,diagnose lio_state = " << std::setprecision(2) <<(state_inout -
    // StatesGroup()).transpose() << endl;
                                           ...
                                           PointCloudXYZINormal &pcl_out)
{
    ...
    std::sort(pcl_out.points.begin(), pcl_out.points.end(), time_list);
    const double &pcl_end_time = pcl_beg_time + pcl_out.points.back().curvature / double(1000);
    /*std::cout << "[ IMU Process ]: Process lidar from " << pcl_beg_time - g_lidar_star_tim << " to " << pcl_end_time-
       g_lidar_star_tim << ", "
              << meas.imu.size() << " imu msgs from " << imu_beg_time- g_lidar_star_tim << " to " << imu_end_time-
       g_lidar_star_tim
...
// fout<<head->header.stamp.toSec()<<" "<<angvel_avr.transpose()<<" "<<acc_avr.transpose()<<std::endl;
    ...
    for (auto it_kp = IMU_pose.end() - 1; it_kp != IMU_pose.begin(); it_kp--)
    {
        ...
        for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--)
        {
            ...
            Eigen::Vector3d T_ei(pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt + R_i * Lidar_offset_to_IMU -
                                 pos_liD_e);
...
void ImuProcess::Process(const MeasureGroup &meas, StatesGroup &stat, PointCloudXYZINormal::Ptr cur_pcl_un_)
{
    ...
    if (imu_need_init_)
    {
        ...
        if (init_iter_num > MAX_INI_COUNT)
        {
            ...
            ROS_INFO("IMU Initials: Gravity: %.4f %.4f %.4f; state.bias_g: %.4f %.4f %.4f; acc covarience: %.8f %.8f "
                     "%.8f; gry covarience: %.8f %.8f %.8f",
