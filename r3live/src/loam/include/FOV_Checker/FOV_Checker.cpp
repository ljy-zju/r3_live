...
void FOV_Checker::check_fov(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth,
                            vector<BoxPointType> &boxes)
{
    ...
    for (i = start_i; i <= maxn; i++)
    {
        ...
        else
        {
            ...
            {
                start_point =
                    Eigen::Vector3d(floor(center_point(0) / box_length + eps_value) * box_length, center_point(1),
                                    floor(center_point(2) / box_length + eps_value) * box_length);
            ...
            {
                start_point =
                    Eigen::Vector3d(floor(center_point(0) / box_length + eps_value) * box_length,
        ...
        for (j = 1; j <= max_vN; j++)
        {
            ...
            u_min = 0;
            box_p_min =
                start_point.cwiseProduct(plane_w + plane_v) + plane_u * plane_u_min + plane_v * box_length * (j - 1);
            box_p_max = plane_u * plane_u_max + start_point.cwiseProduct(plane_w + plane_v) + plane_v * box_length * j +
                        plane_w * box_length;
            // printf("---- UPSIDE
            ...
            while (k >= 0)
            {
                box_p = box_p_min + plane_u * box_length * (u_min + pow(2, k)) + plane_v * box_length +
                        plane_w * box_length;
            ...
            while (k >= 0)
            {
                box_p = box_p_max - plane_u * box_length * (u_max + pow(2, k)) - plane_v * box_length -
                        plane_w * box_length;
        ...
        for (j = 1; j <= max_vN; j++)
        {
            ...
            box_p_max = plane_u * plane_u_max + start_point.cwiseProduct(plane_w + plane_v) -
                        plane_v * box_length * (j - 1) + plane_w * box_length;
            // printf("---- DOWNSIDE
            // (%0.3f,%0.3f,%0.3f),(%0.3f,%0.3f,%0.3f)\n",box_p_min[0],box_p_min[1],box_p_min[2],box_p_max[0],box_p_max[1],box_p_max[2]);
...
bool FOV_Checker::check_box(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth,
                            const BoxPointType box)
{
    ...
    if (check_surface(cur_pose, axis, theta, depth, plane[0]) ||
        check_surface(cur_pose, axis, theta, depth, plane[1]) ||
        check_surface(cur_pose, axis, theta, depth, plane[2]) ||
        check_surface(cur_pose, axis, theta, depth, plane[3]) ||
...
bool FOV_Checker::check_surface(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth,
                                PlaneType plane)
{
    ...
    if (check_line(cur_pose, axis, theta, depth, plane_p, plane_u) ||
        check_line(cur_pose, axis, theta, depth, plane_p, plane_v) ||
        check_line(cur_pose, axis, theta, depth, plane_p + plane_u, plane_v) ||
        check_line(cur_pose, axis, theta, depth, plane_p + plane_v, plane_u))
    {
        s = 1;
        return s;
    }
    ...
    if (t >= -eps_value && t <= depth && vec_dot_u >= -eps_value && vec_dot_u <= plane_u.norm() &&
        vec_dot_v >= -eps_value && vec_dot_v <= plane_v.norm())
        s = 1;
    else
        s = 0;
    return s;
...
bool FOV_Checker::check_line(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth,
                             Eigen::Vector3d line_p, Eigen::Vector3d line_vec)
{
    ...
    if (!(fabs(A) <= eps_value))
    {
        ...
        else
        {
            ...
            if ((t1 >= -eps_value && t1 <= 1 + eps_value) &&
                check_point(cur_pose, axis, theta, depth, line_p + line_vec * t1))
            {
                s = true;
                return s;
            ...
            if ((t2 >= -eps_value && t2 <= 1 + eps_value) &&
                check_point(cur_pose, axis, theta, depth, line_p + line_vec * t2))
            {
                s = true;
                return s;
...
bool FOV_Checker::check_point(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth,
