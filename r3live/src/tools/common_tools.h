...
{
template <typename T> static std::vector<T> vector_2d_to_1d(std::vector<std::vector<T>> &pt_vec_vec)
{
    std::vector<T> pt_vec;
...
template <typename T> std::vector<T> std_set_to_vector(const std::set<T> &input_set)
{
...
template <typename T> std::set<T> std_vector_to_set(const std::vector<T> &input_vector)
{
...
template <typename T> void maintain_maximum_thread_pool(std::list<T> &thread_pool, size_t maximum_parallel_thread)
{
